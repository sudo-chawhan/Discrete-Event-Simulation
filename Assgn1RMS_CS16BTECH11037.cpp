...
bool compare_C(time_block a, time_block b){
	...
	else if(a.time==b.time && a.p.p<b.p.p) return true;
	return false; 
}
...
int main(){
	...
	ofstream output_log ;
	output_log.open("RMS_log.txt",ios::out);
	ofstream output_stat;
	output_stat.open("RMS_stat.txt",ios::out);
	...
	for(int tm=0;tm<2*max_time+100;tm++){
		// output_log<<"\nprocess\tex_time\tturn_time\n";
		// for(int i=0;i<output.size();i++){
		// 	output_log<<output.at(i).pid<<"\t"<<output.at(i).ex<<"\t"<<output.at(i).turn<<endl;
		// }
...
// process is finishing
		...
		if(ready_queue.size()!=0) {
			...
			/////else print_critical_queue(critical_queue);
				///////*****whole thing if cpu is ideal then what to do
...
// current process is being preempted, new process is starting
					if(!new_process.last){
						...
						if(current_process.priority>new_process.priority){
							...
							output_log<<"Process "<<current_process.pid<<": preempted by Process"<<new_process.pid<<" at time "<<tm<<": time left for completion "<<current_process.time_left<<endl;
...
// resumes execution
	...
	output_average_times<<"average waiting time for rms: "<<avg_wait<<endl;
	output_average_times<<"average turnaround time for rms: "<<avg_turn<<endl;
